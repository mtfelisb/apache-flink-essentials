...
package object Gaming {
  ...
  case class Player(playerId: UUID, nickname: String) {
    ...
    def lookingForAGame(startTime: Instant, d: FiniteDuration, gameType: GameType): PlayerIsLookingForAGame =
      PlayerIsLookingForAGame(startTime.plusMillis(d.toMillis), playerId, gameType)
  }
